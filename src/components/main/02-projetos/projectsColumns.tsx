  ...
  return (
    <div className="flex gap-3 text-zinc-500">
        ...
        >
          {projetos.map((projeto) => (
            <motion.div
              ...
              className="bg-white dark:bg-zinc-900 rounded-xl flex flex-col w-[340px] md:w-[400px] p-6 items-center justify-center gap-4 relative shadow-lg cursor-pointer border border-transparent transition-all duration-300 hover:-translate-y-1 hover:border-violet-500/70 hover:shadow-[0_16px_45px_-22px_rgba(102,32,255,0.95)]"
              onClick={() => openProjectDetails(projeto.id)}
              onKeyDown={(event) => {
                if (event.key === "Enter" || event.key === " ") {
                  event.preventDefault();
                  openProjectDetails(projeto.id);
                }
              }}
              tabIndex={0}
              role="button"
              aria-label={`Abrir detalhes do projeto ${projeto.name}`}
            >
              {/* Imagem do projeto */}
      ...
      {activeIndex === 1 && (
        ...
        >
          {projetos
            .filter((projeto) => projeto.software === true) // Filtra apenas projetos com software === true
            .map(
              (
      ...
      {activeIndex === 2 && (
        ...
        >
          {projetos
            .filter((projeto) => projeto.design === true) // Filtra apenas projetos com software === true
            .map(
              (
                projeto, // Mapeia os projetos filtrados para o JSX
              ) => (
